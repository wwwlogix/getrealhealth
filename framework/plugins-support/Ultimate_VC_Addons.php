 ...
 *
 * @link http://codecanyon.net/item/ultimate-addons-for-visual-composer/6892199?ref=wwwlogix
 */
...
add_action( 'init', 'wlgx_sanitize_ultimate_addons', 20 );
function wlgx_sanitize_ultimate_addons() {
...
add_action( 'admin_init', 'wlgx_ultimate_addons_for_vc_integration' );
function wlgx_ultimate_addons_for_vc_integration() {
	if ( get_option( 'ultimate_updater' ) != 'disabled' ) {
		update_option( 'ultimate_updater', 'disabled' );
	}
}

add_action( 'core_upgrade_preamble', 'wlgx_ultimate_addons_core_upgrade_preamble' );
function wlgx_ultimate_addons_core_upgrade_preamble() {
