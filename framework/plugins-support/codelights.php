...
add_action( 'vc_after_set_mode', 'wlgx_provide_codelights_support' );
function wlgx_provide_codelights_support() {
	...
	foreach ( cl_config( 'elements' ) as $name => $elm ) {
		$vc_elm = array(
			...
			'class' => 'elm-' . $name,
			'category' => isset( $elm['category'] ) ? $elm['category'] : wlgx_translate_with_external_domain( 'Content', 'js_composer' ),
...
add_filter( 'cl_image_sizes_select_values', 'wlgx_add_image_sizes_to_codelights' );
function wlgx_add_image_sizes_to_codelights( $sizes ) {
	return array_flip( wlgx_image_sizes_select_values() );
}