...
if ( ! vc_is_page_editable() ) {
	...
	vc_add_params(
		...
		array(
			...
			'type' => 'dropdown',
			'value' => array(
				__( 'Before title', 'wlgx' ) => 'left',
				__( 'After title', 'wlgx' ) => 'right',
			),
		...
		array(
			'param_name' => 'active',
			'heading' => __( 'Active', 'wlgx' ),
			'type' => 'checkbox',
			'value' => array( __( 'Show this section when the page loads', 'wlgx' ) => TRUE ),
			( ( $config['atts']['active'] !== FALSE ) ? 'std' : '_std' ) => $config['atts']['active'],
			'weight' => 50,
		),
		array(
			'param_name' => 'indents',
			'heading' => __( 'Full Size Content', 'wlgx' ),
			'type' => 'checkbox',
			'value' => array( __( 'Remove paddings in the section\'s content area', 'wlgx' ) => 'none' ),
