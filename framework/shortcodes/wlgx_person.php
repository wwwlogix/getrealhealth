...
$atts = wlgx_shortcode_atts( $atts, 'wlgx_person' );
...
if ( ! empty( $atts['custom_icon'] ) AND ! empty( $atts['custom_link'] ) ) {
	$links_html .= '<a class="w-person-links-item" href="' . esc_url( $atts['custom_link'] ) . '" target="_blank">' . wlgx_prepare_icon_tag( $atts['custom_icon'] ) . '</a>';
}
if ( ! empty( $links_html ) ) {
...
$link_start = $link_end = '';
$link = wlgx_vc_build_link( $atts['link'] );
