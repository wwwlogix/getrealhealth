 ...
 *
 * @filter 'wlgx_gmaps_js_options' Allows to filter options, passed to JavaScript
 */
$atts = wlgx_shortcode_atts( $atts, 'wlgx_gmaps' );
...
global $wlgx_gmaps_index;
// Map indexes start from 1
$wlgx_gmaps_index = isset( $wlgx_gmaps_index ) ? ( $wlgx_gmaps_index + 1 ) : 1;
...
if ( $atts['api_key'] != '' ) {
	wp_register_script( 'wlgx-google-maps-with-key', '//maps.googleapis.com/maps/api/js?key=' . $atts['api_key'], array(), '', FALSE );
	wp_enqueue_script( 'wlgx-google-maps-with-key' );
} else {
	wp_enqueue_script( 'wlgx-google-maps' );
}
if ( wlgx_get_option( 'ajax_load_js', 0 ) == 0 ) {
	wp_enqueue_script( 'wlgx-gmap' );
}

$script_options = apply_filters( 'wlgx_gmaps_js_options', $script_options, get_the_ID(), $wlgx_gmaps_index );

$output = '<div class="w-map' . $classes . '" id="wlgx_map_' . $wlgx_gmaps_index . '"' . $inner_css . '>';
$output .= '<div class="w-map-h"></div>';
$output .= '<div class="w-map-json"' . wlgx_pass_data_to_js( $script_options ) . '></div>';
// Style JSON
