...
$atts = wlgx_shortcode_atts( $atts, 'wlgx_pricing' );
...
foreach ( $atts['items'] as $index => $item ) {
	 ...
	 */
	//$item = wlgx_shortcode_atts( array_filter( $item ), 'wlgx_pricing', 'items_atts' );
	...
	if ( ! empty( $item['btn_text'] ) ) {
		...
		if ( $item['btn_icon'] != '' ) {
			$icon_html = wlgx_prepare_icon_tag( $item['btn_icon'] );
		...
		$btn_link = wlgx_vc_build_link( $item['btn_link'] );
