...
global $wlgx_tabs_atts, $wlgx_tab_index;
// Tab indexes start from 1
$wlgx_tab_index = isset( $wlgx_tab_index ) ? ( $wlgx_tab_index + 1 ) : 1;

// We could overload some of the atts at vc_tabs implementation, so apply them in here as well
if ( isset( $wlgx_tab_index ) AND isset( $wlgx_tabs_atts[$wlgx_tab_index - 1] ) ) {
	$atts = array_merge( $atts, $wlgx_tabs_atts[$wlgx_tab_index - 1] );
}

$atts = wlgx_shortcode_atts( $atts, 'vc_tta_section' );
...
if ( $atts['icon'] AND $atts['i_position'] == 'left' ) {
	$output .= wlgx_prepare_icon_tag( $atts['icon'] );
}
