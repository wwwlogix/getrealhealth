 ...
 *
 * Overloaded by wwwlogix custom implementation.
 *
...
$atts = wlgx_shortcode_atts( $atts, $shortcode_base );

$classes = '';
$list_classes = '';

// Extract tab attributes for future html preparations
global $wlgx_tabs_atts;
preg_match_all( '/\[vc_tta_section([^\]]*?)\]/i', $content, $matches, PREG_OFFSET_CAPTURE );
$wlgx_tabs_atts = isset( $matches[0] ) ? $matches[0] : array();
$active_tab_indexes = array();
foreach ( $wlgx_tabs_atts as $index => $tab_atts ) {
	$wlgx_tabs_atts[$index] = shortcode_parse_atts( '[' . rtrim( $tab_atts[0], '[]' ) . ' ]' );
	if ( isset( $wlgx_tabs_atts[$index]['active'] ) AND $wlgx_tabs_atts[$index]['active'] ) {
...
if ( empty( $active_tab_indexes ) AND $shortcode_base != 'vc_tta_accordion' ) {
	...
	$wlgx_tabs_atts[0]['active'] = 'yes';
}

// Inheriging some of the attributes to the sections
if ( isset( $atts['c_position'] ) ) {
	foreach ( $wlgx_tabs_atts as $index => $tab_atts ) {
		$wlgx_tabs_atts[$index]['c_position'] = $atts['c_position'];
	}
}
...
if ( isset( $atts['title_tag'] ) ) {
	foreach ( $wlgx_tabs_atts as $index => $tab_atts ) {
		$wlgx_tabs_atts[$index]['title_tag'] = $atts['title_tag'];
	}
}
...
if ( isset( $atts['title_size'] ) ) {
	foreach ( $wlgx_tabs_atts as $index => $tab_atts ) {
		$wlgx_tabs_atts[$index]['title_size'] = $atts['title_size'];
	}
}
...
$list_classes .= ' items_' . count( $wlgx_tabs_atts );
...
foreach ( $wlgx_tabs_atts as $index => $tab_atts ) {
	...
	if ( isset( $tab_atts['icon'] ) AND $tab_atts['i_position'] == 'left' ) {
		$output .= wlgx_prepare_icon_tag( $tab_atts['icon'] );
	}
...
// Handling inner tabs
global $wlgx_tab_index;
$wlgx_tab_index = 0;

$output .= '<div class="w-tabs-sections"><div class="w-tabs-sections-h">' . do_shortcode( $content ) . '</div></div></div>';

$output .='<div class="responsive-tabs w-tabs"><div class="w-testimonials style_2 cols_3 type_carousel" style="" data-items="1" data-autoplay="0" data-timeout="3000" data-nav="0" data-dots="1" data-autoheight="0">';
