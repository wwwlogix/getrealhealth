...
$atts = wlgx_shortcode_atts( $atts, 'wlgx_image_slider' );

if ( empty( $atts['ids'] ) ) {
	return;
}

global $wlgx_image_slider_index;
// Image sliders indexes start from 1
$wlgx_image_slider_index = isset( $wlgx_image_slider_index ) ? ( $wlgx_image_slider_index + 1 ) : 1;
...
$output .= '<div class="wlgx-frame' . $frame_classes . '">';
$output .= '<div class="royalSlider rsDefault">' . $images_html . '</div>';
$output .= '<div class="w-slider-json"' . wlgx_pass_data_to_js( $js_options ) . '></div>';
$output .= '</div>';
$output .= '</div>';
