...
$atts = wlgx_shortcode_atts( $atts, 'wlgx_testimonial' );
...
if ( ! empty( $image_html ) OR ! empty( $atts['author'] ) OR ! empty( $atts['company'] ) ) {
	...
	if ( ! empty( $atts['author'] ) ) {
		$output .= '<div class="w-testimonial-author-name"><div>' . $atts['author'] . '</div></div>';
	}
	if ( ! empty( $atts['company'] ) ) {
		$output .= '<div class="w-testimonial-author-role">' . $atts['company'] . '</div>';
	}
