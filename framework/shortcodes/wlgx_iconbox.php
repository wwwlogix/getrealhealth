 ...
 */
$atts = wlgx_shortcode_atts( $atts, 'wlgx_iconbox' );
...
if ( $atts['img'] != '' ) {
	...
	if ( $atts['icon'] != '' ) {
		$icon_html = wlgx_prepare_icon_tag( $atts['icon'] );
	}
}
...
if ( ! empty( $link['url'] ) ) {
	...
	$link_opener = '<a class="w-iconbox-link" href="' . esc_url( $link['url'] ) . '"' . $link_target . $link_rel . $link_title . '>'.$link['title'];
	$link_closer = '</a>';
}
