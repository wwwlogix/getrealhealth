...
// Translating attributes from [gallery] to [wlgx_gallery] format (may be used in both shortcodes
$link_type = 'media';
...
$atts = wlgx_shortcode_atts( $atts, 'wlgx_gallery' );

if ( empty( $atts['ids'] ) ) {
	return;
}

global $wlgx_gallery_index;
// Gallery indexes start from 1
$wlgx_gallery_index = isset( $wlgx_gallery_index ) ? ( $wlgx_gallery_index + 1 ) : 1;
...
$classes = apply_filters( 'wlgx_gallery_listing_classes', $classes );
...
foreach ( $attachments as $index => $attachment ) {
	...
	$output .= apply_filters( 'wlgx_gallery_listing_item_classes', '' );
	$output .= '"';
	if ( $link_type == 'media' ) {
