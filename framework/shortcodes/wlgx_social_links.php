...
$atts = wlgx_shortcode_atts( $atts, 'wlgx_social_links' );

$socials = wlgx_config( 'social_links' );

$socials = array_merge( array( 'email' => 'Email' ), $socials );

$classes = '';

global $wlgx_socials_index;
// Social links indexes indexes start from 1
$wlgx_socials_index = isset( $wlgx_socials_index ) ? ( $wlgx_socials_index + 1 ) : 1;
...
$classes .= ' index_' . $wlgx_socials_index;
if ( $atts['el_class'] != '' ) {
...
if ( ! empty( $atts['custom_icon'] ) AND ! empty( $atts['custom_link'] ) ) {
	...
	$output .= wlgx_prepare_icon_tag( $atts['custom_icon'] );
	$output .= '</a>';
