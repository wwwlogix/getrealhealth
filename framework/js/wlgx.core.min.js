...
if($us.header.orientation=='hor'&&this.type=='mobile'&&$us.canvas.headerPos=='fixed')this.setFixedMobileMaxHeight();this.$list.removeClass('hidden')}};$us.nav=new USNav()}(jQuery);!function($){"use strict";function USScroll(options){var defaults={attachOnInit:'.w-nav a[href*="#"], .w-menu a[href*="#"], a.w-btn[href*="#"], .w-iconbox a[href*="#"], .w-image a[href*="#"], .w-img a[href*="#"], .w-text a[href*="#"], '+'.vc_icon_element a[href*="#"], .vc_custom_heading a[href*="#"], a.w-casestudy-item-anchor[href*="#"], .widget_nav_menu a[href*="#"], .w-toplink, '+'.w-blog-post-meta-comments a[href*="#"], .w-comments-title a[href*="#"], .w-comments-item-date, a.smooth-scroll[href*="#"]',buttonActiveClass:'active',menuItemActiveClass:'current-menu-item',menuItemAncestorActiveClass:'current-menu-ancestor',animationDuration:1200,animationEasing:'easeInOutQuint'};this.options=$.extend({},defaults,options||{});this.blocks={};this.isScrolling=!1;this.waypoints=[];this._events={cancel:this.cancel.bind(this),scroll:this.scroll.bind(this),resize:this.resize.bind(this)};this._canvasTopOffset=0;$us.$window.on('resize load',this._events.resize);setTimeout(this._events.resize,75);$us.$window.on('scroll',this._events.scroll);setTimeout(this._events.scroll,75);if(this.options.attachOnInit){this.attach(this.options.attachOnInit)}
...
$us.$canvas.on('contentChange',function(){$slider.parent().imagesLoaded(function(){slider.updateSliderSize()})})}.bind(this))})};$(function(){jQuery('.w-slider').wSlider()})})(jQuery);!function($){"use strict";$us.Wcasestudy=function(container,options){this.init(container,options)};$us.Wcasestudy.prototype={init:function(container,options){this.$container=$(container);if(this.$container.usMod('position')!='isotope'){return}
this.$filters=this.$container.find('.g-filters-item');this.$list=this.$container.find('.w-casestudy-list');this.$items=this.$container.find('.w-casestudy-item');this.$pagination=this.$container.find('.g-pagination');this.$loadmore=this.$container.find('.g-loadmore');this.paginationType=this.$pagination.length?'regular':(this.$loadmore.length?'ajax':'none');this.items={};this.curCategory='*';this.loading=!1;this.$items.each(function(index,item){this.items[parseInt(item.getAttribute('data-id'))]=$(item)}.bind(this));this.isotopeOptions={itemSelector:'.w-casestudy-item',layoutMode:'masonry',masonry:{},isOriginLeft:!$('.l-body').hasClass('rtl')};if(this.$container.find('.w-casestudy-item.size_1x1').length){this.itemWidth=1;this.isotopeOptions.masonry.columnWidth='.size_1x1'}else if(this.$container.find('.w-casestudy-item.size_1x2').length){this.itemWidth=1;this.isotopeOptions.masonry.columnWidth='.size_1x2'}else{this.itemWidth=2;this.isotopeOptions.masonry.columnWidth='.w-casestudy-item'}
if(this.paginationType!='none'){var $jsonContainer=this.$container.find('.w-casestudy-json');if($jsonContainer.length==0)return;this.jsonData=$jsonContainer[0].onclick()||{};this.ajaxUrl=this.jsonData.ajax_url||'';this.templateVars=JSON.stringify(this.jsonData.template_vars||{});this.perpage=this.jsonData.perpage||this.$items.length;this.order=this.jsonData.order||{};this.sizes=this.jsonData.sizes||{};this.curPage=this.jsonData.page||1;this.infiniteScroll=this.jsonData.infinite_scroll||0;$jsonContainer.remove();this.isotopeOptions.sortBy='number';this.isotopeOptions.getSortData={number:function(elm){return this.order['*'].indexOf(parseInt(elm.getAttribute('data-id')))}.bind(this)}}
...
if(this.loading)return;category=category||this.curCategory;var start=(this.paginationType=='ajax')?0:((page-1)*this.perpage),length=page*this.perpage,showIds=(this.order[category]||[]).slice(start,length),loadIds=[],$newItems=[];$.each(showIds,function(i,id){if(this.items[id]!==undefined)return;var itemSize=(this.sizes[id]||'1x1'),itemHtml='<div class="w-casestudy-item size_'+itemSize+' loading" data-id="'+id+'">'+'<div class="w-casestudy-item-anchor"><div class="g-preloader type_1"></div></div></div>';this.items[id]=$(itemHtml).appendTo(this.$list);$newItems.push(this.items[id][0]);loadIds.push(showIds[i])}.bind(this));if(loadIds.length>0){var $insertedItems=$();$.ajax({type:'post',url:this.ajaxUrl,data:{action:'wlgx_ajax_casestudy',ids:loadIds.join(','),template_vars:this.templateVars},success:function(html){var $container=$('<div>',{html:html}),$items=$container.children(),isotope=this.$list.data('isotope');$items.each(function(index,item){var $item=$(item),itemID=parseInt($item.data('id'));$item.imagesLoaded(function(){this.items[itemID].attr('class',$item.attr('class')).attr('style',$item.attr('style'));this.itemLoaded(itemID,$item);this.items[itemID].html($item.html());$insertedItems=$insertedItems.add(this.items[itemID]);if($insertedItems.length>=loadIds.length){$container.remove();this.itemsLoaded($insertedItems)}
if(isotope){if(this.itemWidth!=1){if(this.$container.find('.w-casestudy-item.size_1x1').length){this.itemWidth=1;this.isotopeOptions.masonry.columnWidth='.size_1x1'}else if(this.$container.find('.w-casestudy-item.size_1x2').length){this.itemWidth=1;this.isotopeOptions.masonry.columnWidth='.size_1x2'}else{this.itemWidth=2;this.isotopeOptions.masonry.columnWidth='.w-casestudy-item'}
