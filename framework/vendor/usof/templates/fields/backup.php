...
if ( $backup AND is_array( $backup ) AND isset( $backup['time'] ) ) {
	...
	$output .= __( 'Last Backup', 'wlgx' ) . ': <span>' . date_i18n( 'F j, Y, g:i a', $backup_time ) . '</span>';
} else {
	$output .= __( 'No backups yet', 'wlgx' );
}
$output .= '</div>';
$output .= '<div class="usof-button type_backup"><span>' . __( 'Backup Options', 'wlgx' ) . '</span></div>';
$output .= '<div class="usof-button type_restore"';
if ( ! $backup OR ! is_array( $backup ) OR ! isset( $backup['usof_options'] ) ) {
	$output .= ' style="display: none"';
}
$output .= '><span class="usof-button-label">' . __( 'Restore Options', 'wlgx' ) . '</span></div>';
$i18n = array(
	'restore_confirm' => __( 'Are you sure want to restore options from the backup?', 'wlgx' ),
);
$output .= '<div class="usof-backup-i18n"' . wlgx_pass_data_to_js( $i18n ) . '></div>';
$output .= '</div>';
