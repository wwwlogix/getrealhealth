...
function usof_ajax_save() {
	...
	if ( ! check_admin_referer( 'usof-actions' ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Cannot verify your request, plese refresh the page', 'wlgx' ),
			)
		);
	}
	...
	$post_options = wlgx_maybe_get_post_json( 'usof_options' );

	if ( empty( $post_options ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'There\'s no options to save', 'wlgx' ),
			)
		);
	}
	...
	wp_send_json_success(
		array(
			'message' => __( 'Options were saved', 'wlgx' ),
		)
	);
}
...
function usof_ajax_reset() {
	...
	wp_send_json_success(
		array(
			'message' => __( 'Options were reset', 'wlgx' ),
...
function usof_ajax_backup() {
	...
	wp_send_json_success(
		array(
			'message' => __( 'Backup was created', 'wlgx' ),
			'status' => __( 'Last Backup', 'wlgx' ) . ': <span>' . date_i18n( 'F j, Y, g:i a', $backup_time ) . '</span>',
		)
	);
}
...
function usof_ajax_restore_backup() {
	...
	if ( ! $backup OR ! is_array( $backup ) OR ! isset( $backup['usof_options'] ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'There\'s no backup to restore', 'wlgx' ),
			)
		);
	}
	...
	wp_send_json_success(
		array(
			'message' => __( 'Backup was restored', 'wlgx' ),
			'usof_options' => $usof_options,
		)
	);
}
...
function usof_ajax_save_style_scheme() {
	...
	$scheme = wlgx_maybe_get_post_json( 'scheme' );
	if ( isset( $scheme[id] ) ) {
		$scheme_id = $scheme[id];
	} else {
		$max_index = 0;
