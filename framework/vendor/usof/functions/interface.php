...
add_action( 'admin_menu', 'wlgx_options_admin_menu', 9 );
function wlgx_options_admin_menu() {
	add_menu_page( __( 'Theme Options', 'wlgx' ), wlgx_THEMENAME, 'manage_options', 'wlgx-theme-options', 'wlgx_theme_options_page', NULL, '59.001' );
	$usof_page = add_submenu_page( 'wlgx-theme-options', wlgx_THEMENAME, __( 'Theme Options', 'wlgx' ), 'edit_theme_options', 'wlgx-theme-options', 'wlgx_theme_options_page' );
...
function wlgx_theme_options_page() {
	...
	$config = wlgx_config( 'theme-options', array() );
	echo '<div class="usof-container';
	echo apply_filters( 'usof_container_classes', '' );
	if ( get_option( 'wlgx_license_activated', 0 ) ) {
	...
	echo wlgx_THEMENAME . ' <span class="version">' . wlgx_THEMEVERSION . '</span><span class="dash">&mdash;</span></div>';
	echo '<div class="usof-header-title"><h2>' . __( 'General Settings', 'wlgx' ) . '</h2></div>';
	echo '<div class="usof-control for_save status_clear">';
	echo '<button class="usof-button type_save" type="button"><span>' . __( 'Save Changes', 'wlgx' ) . '</span>';
	...
	foreach ( $config as $section_id => &$section ) {
		...
		if ( isset( $section['new'] ) AND $section['new'] AND ! in_array( $section_id, $visited_new_sections ) ) {
			echo '<span class="usof-nav-popup">' . __( 'New', 'wlgx' ) . '</span>';
		}
		echo '</li>';
	}
	echo '<ul></div>';
	...
	echo '</div>';
	echo '<div class="usof-hidden-fields"' . wlgx_pass_data_to_js( $hidden_fields_values ) . '></div>';
}
...
function usof_print_styles() {
	global $usof_directory_uri, $usof_version, $wlgx_template_directory_uri;
	wp_enqueue_style( 'font-awesome', $wlgx_template_directory_uri . '/framework/css/font-awesome.css', array(), '4.7.0' );
