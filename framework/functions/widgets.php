...
abstract class wlgx_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		if ( empty( $id_base ) ) {
			// ... or fetched from the wlgx_Widget_<name> part
			$id_base = preg_replace( '/(wp_)?widget_/', '', strtolower( get_class( $this ) ) );
		}

		$this->config = wlgx_config( 'widgets.' . $id_base );
		if ( $this->config === NULL ) {
			wp_die( 'Wrong wwwlogix widget definition: id_base should be set by attribute or by class name' );
		}
...
add_action( 'widgets_init', 'wlgx_widgets_init' );
function wlgx_widgets_init() {
	$config = wlgx_config( 'widgets', array() );
	foreach ( $config as $widget_id => $widget ) {
		if ( ! isset( $widget['class'] ) ) {
			wp_die( 'Widget class should be defined for ' . $widget_id );
		}
		if ( ! class_exists( $widget['class'] ) ) {
			$filepath = wlgx_locate_file( 'widgets/' . $widget_id . '.php' );
			if ( $filepath === FALSE ) {
				continue;
			}
			include $filepath;
