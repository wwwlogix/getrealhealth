 ...
 */
function wlgx_breadcrumbs() {

	/* === OPTIONS === */
	$text['home'] = __( 'Home', 'wlgx' ); // text for the 'Home' link
	$text['category'] = __( 'Archive by Category "%s"', 'wlgx' ); // text for a category page
	$text['search'] = __( 'Search Results for "%s" Query', 'wlgx' ); // text for a search results page
	$text['tag'] = __( 'Posts Tagged "%s"', 'wlgx' ); // text for a tag page
	$text['author'] = __( 'Articles Posted by %s', 'wlgx' ); // text for an author page
	$text['404'] = __( 'Error 404', 'wlgx' ); // text for the 404 page
	$text['forums'] = __( 'Forums', 'wlgx' ); // text for the 404 page
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( is_category() ) {
			...
			echo sprintf( $link, get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ), __( get_the_time( 'F' ), 'wlgx' ) ) . $delimiter;
			echo $before . get_the_time( 'd' ) . $after;
		} elseif ( is_month() ) {
			echo sprintf( $link, get_year_link( get_the_time( 'Y' ) ), get_the_time( 'Y' ) ) . $delimiter;
			echo $before . __( get_the_time( 'F' ), 'wlgx' ) . $after;
		} elseif ( is_year() ) {
			...
			if ( isset( $post_type->labels->name ) ) {
				...
				if ( function_exists( 'bbp_is_single_user' ) and bbp_is_single_user() ) {
					echo wlgx_translate_with_external_domain( 'Profile', 'bbpress' );
				} else {
		...
		if ( get_query_var( 'paged' ) AND ! ( get_post_type() == 'topic' OR get_post_type() == 'forum' ) ) {
			...
			echo __( 'Page', 'wlgx' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ')';
			}
		}
