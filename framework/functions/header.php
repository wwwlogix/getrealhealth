 ...
 * @return string
 */
function wlgx_get_header_option( $name, $state = 'default', $default = NULL ) {
	...
	if ( $state != 'default' AND ( ! isset( $wlgx_header_settings[$state]['options'][$name] ) OR in_array( $name, $shared_options ) ) ) {
		$state = 'default';
	}

	return isset( $wlgx_header_settings[$state]['options'][$name] ) ? $wlgx_header_settings[$state]['options'][$name] : $default;
}
 ...
 * @return array
 */
function wlgx_get_header_layout( $state = 'default' ) {
	global $wlgx_header_settings;
	wlgx_load_header_settings_once();
	$layout = array(
		'top_left' => array(),
	...
	if ( $state != 'default' AND isset( $wlgx_header_settings['default']['layout'] ) AND is_array( $wlgx_header_settings['default']['layout'] ) ) {
		$layout = array_merge( $layout, $wlgx_header_settings['default']['layout'] );
	}
	if ( isset( $wlgx_header_settings[$state]['layout'] ) AND is_array( $wlgx_header_settings[$state]['layout'] ) ) {
		$layout = array_merge( $layout, $wlgx_header_settings[$state]['layout'] );
	}
 ...
 */
function wlgx_load_header_settings_once() {

	global $wlgx_header_settings;

	if ( isset( $wlgx_header_settings ) ) {
		return;
	}
	// Basic structure
	$wlgx_header_settings = array(
		'default' => array( 'options' => array(), 'layout' => array() ),
		'tablets' => array( 'options' => array(), 'layout' => array() ),
		'mobiles' => array( 'options' => array(), 'layout' => array() ),
		'data' => array(),
	);
	$wlgx_header_settings = apply_filters( 'wlgx_load_header_settings', $wlgx_header_settings );
}

/**
 * Translating the v.2 USOF header settings to v.3 header-builder compatible ones
 */
add_filter( 'wlgx_load_header_settings', 'wlgx_load_usof_header_settings' );
function wlgx_load_usof_header_settings( $header_settings ) {

	global $usof_options;
	usof_load_options_once();

	// Side options defaults from theme-options.php config
	$side_options_config = wlgx_config( 'header-settings.options', array() );
	...
	$header_templates = wlgx_config( 'header-templates', array() );
	if ( isset( $usof_options['header_layout'] ) AND isset( $header_templates[$usof_options['header_layout']] ) ) {
		$header_template = wlgx_fix_header_template_settings( $header_templates[$usof_options['header_layout']] );
		$header_settings = wlgx_array_merge( $header_settings, $header_template );
	}

	// Filling elements' data with default values
	$header_settings = wlgx_fix_header_settings( $header_settings );
	...
	$elms_config = wlgx_config( 'header-settings.elements', array() );
...
add_filter( 'wlgx_load_header_settings', 'wlgx_load_metabox_header_settings', 20 );
function wlgx_load_metabox_header_settings( $header_settings ) {
	if ( is_singular( array( 'post', 'page', 'wlgx_casestudy', 'product' ) ) ) {
		foreach ( array( 'default', 'tablets', 'mobiles' ) as $state ) {
			if ( wlgx_arr_path( $header_settings, $state . '.options.orientation' ) == 'hor' ) {
				if ( usof_meta( 'wlgx_header_bg' ) != '' ) {
					$header_settings[$state]['options']['transparent'] = intval( usof_meta( 'wlgx_header_bg' ) == 'transparent' );
				}
				if ( usof_meta( 'wlgx_header_pos' ) != '' ) {
					$header_settings[$state]['options']['sticky'] = intval( usof_meta( 'wlgx_header_pos' ) == 'fixed' );
				}
			}
		}
	}
 ...
 */
function wlgx_output_header_elms( &$layout, &$data, $place ) {
	...
	foreach ( $layout[$place] as $elm ) {
		if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
			...
			if ( isset( $data[$elm] ) ) {
				...
				if ( wlgx_arr_path( $data[$elm], 'design_options.hide_for_sticky', FALSE ) ) {
					$classes .= ' hide-for-sticky';
				}
			}
			echo '<div class="w-' . $type . $classes . ' ush_' . str_replace( ':', '_', $elm ) . '">';
			wlgx_output_header_elms( $layout, $data, $elm );
			echo '</div>';
		} else {
			// Element
			$type = strtok( $elm, ':' );
			$defaults = wlgx_get_header_elm_defaults( $type );
			if ( ! isset( $data[$elm] ) ) {
				$data[$elm] = wlgx_get_header_elm_defaults( $type );
			}
			$values = array_merge( $defaults, array_intersect_key( $data[$elm], $defaults ) );
			$values['id'] = $elm;
			wlgx_load_template( 'templates/elements/' . $type, $values );
		}
	}
}

function wlgx_get_header_elm_defaults( $type ) {
	global $wlgx_header_elm_defaults, $usof_options;
	usof_load_options_once();
	if ( ! isset( $wlgx_header_elm_defaults ) ) {
		$wlgx_header_elm_defaults = array();
	}
	if ( ! isset( $wlgx_header_elm_defaults[$type] ) ) {
		foreach ( wlgx_config( 'header-settings.elements.' . $type . '.params', array() ) as $field_name => $field ) {
			...
			$wlgx_header_elm_defaults[$type][$field_name] = $value;
		}
	}

	return $wlgx_header_elm_defaults[$type];
}
 ...
 * @return array
 */
function wlgx_get_header_elms_of_a_type( $type, $key_as_class = TRUE ) {
	global $wlgx_header_settings;
	wlgx_load_header_settings_once();
	$defaults = wlgx_get_header_elm_defaults( $type );
	$result = array();
	foreach ( $wlgx_header_settings['data'] as $elm_id => $elm ) {
 ...
 * @return array
 */
function wlgx_fix_header_settings( $value ) {
	...
	foreach ( wlgx_config( 'header-settings.options', array() ) as $group => $opts ) {
...
function wlgx_fix_header_template_settings( $value ) {
	...
	$value = wlgx_array_merge( $template_structure, $value );
	...
	$value = wlgx_fix_header_settings( $value );
 ...
 * @return array
 */
function wlgx_get_header_shown_elements_list( $list ) {
	$shown = array();
...
add_action( 'wp_footer', 'wlgx_pass_header_settings_to_js' );
function wlgx_pass_header_settings_to_js() {
	global $wlgx_header_settings;
	wlgx_load_header_settings_once();
	$header_settings = $wlgx_header_settings;
 ...
 * @return string
 */
function wlgx_get_header_design_options_css() {
	global $wlgx_header_settings;
	wlgx_load_header_settings_once();
	$sizes = array(
 ...
 * @param $wp_admin_bar
 */
function wlgx_admin_bar_theme_options_link( $wp_admin_bar ) {
	$args = array(
		'id' => 'wlgx_theme_otions',
		'title' => __( 'Theme Options', 'wlgx' ),
		'href' => admin_url( 'admin.php?page=wlgx-theme-options' ),
		'parent' => 'site-name',
		'meta' => array(
			'class' => 'wlgx-theme-options',
			'title' => __( 'Theme Options', 'wlgx' ),
		),
	);
...
if ( ! is_admin() ) {
	add_action( 'admin_bar_menu', 'wlgx_admin_bar_theme_options_link', 99 );
