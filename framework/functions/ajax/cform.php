...
/**
 * Ajax method for sending contact form via wlgx_cform shortcode
 */
add_action( 'wp_ajax_nopriv_wlgx_ajax_cform', 'wlgx_ajax_cform' );
add_action( 'wp_ajax_wlgx_ajax_cform', 'wlgx_ajax_cform' );
function wlgx_ajax_cform() {
	...
	preg_match_all( '~\[(vc_contact_form|wlgx_cform)(.*?)\]~', $post_content, $matches );
	...
	// Validating fields
	$errors = array();
	$fields = wlgx_config( 'cform.fields', array() );
	...
	if ( $filled_fields_count == 0 ) {
		$errors['empty_message'] = wlgx_config( 'cform.error.empty_message', '' );
	}
	...
	if ( isset( $fields['name'] ) AND isset( $shortcode_atts['name_field'] ) AND $shortcode_atts['name_field'] != 'hidden' AND $_POST['name'] != '' ) {
		...
		$body_name_part = __( 'Someone', 'wlgx' );
	}

	if ( isset( $fields['email'] ) AND isset( $shortcode_atts['email_field'] ) AND $shortcode_atts['email_field'] != 'hidden' AND $_POST['email'] != '' ) {
		if ( is_rtl() ) {
			$body_email_part = '<b>' . sanitize_email( stripslashes( $_POST['email'] ) ) . '</b> :' . __( 'Email', 'wlgx' );
		} else {
			$body_email_part = __( 'Email', 'wlgx' ) . ': <b>' . sanitize_email( stripslashes( $_POST['email'] ) ) . '</b>';
		}
	...
	if ( isset( $fields['phone'] ) AND isset( $shortcode_atts['phone_field'] ) AND $shortcode_atts['phone_field'] != 'hidden' AND $_POST['phone'] != '' ) {
		if ( is_rtl() ) {
			$body_phone_part = '<b>' . sanitize_text_field( stripslashes( $_POST['phone'] ) ) . '</b> :' . __( 'Phone Number', 'wlgx' );
		} else {
			$body_phone_part = __( 'Phone Number', 'wlgx' ) . ': <b>' . sanitize_text_field( stripslashes( $_POST['phone'] ) ) . '</b>';
		}
	} else {
		$body_phone_part = '';
	}
	...
	$email_body = '<p>' . __( 'You received a message from', 'wlgx' ) . ' <a href="' . site_url() . '">' . get_bloginfo( 'name' ) . '</a></p>';
	if ( is_rtl() ) {
		$email_body .= '<p>';
		$email_body .= ( $body_message_part != '' ) ? ':' : '.';
		$email_body .= _x( 'wrote', 'smb wrote a message', 'wlgx' ) . $body_contacts_part . $body_name_part . '</p>';
	} else {
		$email_body .= '<p>' . $body_name_part . $body_contacts_part . _x( 'wrote', 'smb wrote a message', 'wlgx' );
		$email_body .= ( $body_message_part != '' ) ? ':' : '.';
		$email_body .= '</p>';
	}
	$email_body .= $body_message_part;

	$email_subject = sprintf( wlgx_config( 'cform.subject' ), get_bloginfo( 'name' ) );

	if ( empty( $email_body ) ) {
		wp_send_json_error( wlgx_config( 'cform.error.empty_message' ) );
	}
	if ( is_rtl() ) {
		$email_body = '<div style="direction: rtl; unicode-bidi: embed;">' . $email_body . '</div>';
	}

	add_filter( 'wp_mail_content_type', 'wlgx_set_html_content_type' );
	function wlgx_set_html_content_type() {
		return 'text/html';
	}
	...
	if ( $success ) {
		wp_send_json_success( wlgx_config( 'cform.success' ) );
	} else {
		wp_send_json_error( wlgx_config( 'cform.error.other' ) );
	}
}
