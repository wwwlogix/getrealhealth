...
class wlgx_Shortcodes {
	...
	/**
	 * Singleton pattern: wlgx_Shortcodes::instance()->wlgx_blog($atts, $content)
	 *
	 * @return wlgx_Shortcodes
	 */
	...
	protected function __construct() {
		global $wlgx_template_directory, $wlgx_stylesheet_directory;
		$this->config = wlgx_config( 'shortcodes' );
		...
		$this->_template_directory = $wlgx_template_directory;
		$this->_stylesheet_directory = $wlgx_stylesheet_directory;
	}
	...
	public function init() {
		...
		foreach ( $this->config as $shortcode => $shortcode_params ) {
			if ( isset( $shortcode_params['supported'] ) AND ! $shortcode_params['supported'] ) {
				...
				if ( ! wlgx_get_option( 'enable_unsupported_vc_shortcodes', FALSE ) AND shortcode_exists( $shortcode ) ) {
...
global $wlgx_shortcodes;
$wlgx_shortcodes = wlgx_Shortcodes::instance();

// Add custom options to WP Gallery window
add_action( 'print_media_templates', 'wlgx_media_templates' );
function wlgx_media_templates() {
	?>
	<script type="text/html" id="tmpl-my-custom-gallery-setting">
		<label class="setting">
			<span><?php _e( 'Add indents between items', 'wlgx' ) ?></span>
			<input type="checkbox" data-setting="indents">
		</label>
		<label class="setting">
			<span><?php _e( 'Enable Masonry layout mode', 'wlgx' ) ?></span>
			<input type="checkbox" data-setting="masonry">
		</label>
		<label class="setting">
			<span><?php _e( 'Show items titles and description', 'wlgx' ) ?></span>
