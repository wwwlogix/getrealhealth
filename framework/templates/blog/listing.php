 ...
 *
 * @action Before the template: 'wlgx_before_template:templates/blog/listing'
 * @action After the template: 'wlgx_after_template:templates/blog/listing'
 * @filter Template variables: 'wlgx_template_vars:templates/blog/listing'
 */
...
while ( have_posts() ) {
	...
	wlgx_load_template( 'templates/blog/listing-post', $template_vars );
}
...
if ( $wp_query->max_num_pages > 1 ) {
	...
	if ( wlgx_get_option( 'ajax_load_js', 0 ) == 0 ) {
		wp_enqueue_script( 'wlgx-royalslider' );
	}
	...
	if ( $pagination != 'none' ) {
		?>
		<div class="w-blog-json hidden"<?php echo wlgx_pass_data_to_js( $json_data ) ?>></div>
		<?php
	}
