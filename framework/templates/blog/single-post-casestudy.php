 ...
 *
 * @action Before the template: 'wlgx_before_template:templates/blog/single-post'
 * @action After the template: 'wlgx_after_template:templates/blog/single-post'
 * @filter Template variables: 'wlgx_template_vars:templates/blog/single-post'
 */
...
$preview_type = usof_meta( 'wlgx_post_preview_layout' );
if ( $preview_type == '' ) {
	$preview_type = wlgx_get_option( 'post_preview_layout', 'basic' );
}
...
if ( $preview_type != 'none' AND ! post_password_required() ) {
	...
	if ( $preview_type == 'basic' ) {
		if ( in_array( $post_format, array( 'video', 'gallery', 'audio' ) ) ) {
			$preview_html = wlgx_get_post_preview( $the_content, TRUE );
		...
		if ( $post_thumbnail_id ) {
			...
			$preview_bg_html = wlgx_get_post_preview( $the_content, TRUE );
...
if ( in_array( 'comments', $metas ) AND ! ( $comments_number == 0 AND ! comments_open() ) ) {
	...
	comments_popup_link( wlgx_translate_with_external_domain( 'No Comments' ), $comments_label, $comments_label );
	$meta_html['comments'] .= ob_get_clean();
	$meta_html['comments'] .= '</span>';
}

if ( wlgx_get_option( 'post_nav' ) ) {
...
$meta_html = apply_filters( 'wlgx_single_post_meta_html', $meta_html, get_the_ID() );
	...
	<div class="l-section-h i-cf">
		<div class="w-blog">
			...
			<div class="w-blog-post-body">
				<h1 class="w-casestudy-post-title entry-title" itemprop="headline"><?php the_title() ?></h1>

			</div>
		</div>

		<?php
		if ( $preview_type == 'trendy' AND $wlgx_layout->sidebar_pos == 'none' AND $wlgx_layout->titlebar == 'none' ) {
			add_action( 'wp_footer', 'wlgx_trendy_preview_parallax', 99 );
			function wlgx_trendy_preview_parallax() { ?>
				<script>
					(function($){
						var $window = $(window),
...
<?php if ( $show_tags AND ! empty( $the_tags ) ): ?>
	<section class="l-section for_tags">
		<div class="l-section-h i-cf">
			<div class="g-tags">
				<span class="g-tags-title"><?php _e( 'Tags', 'wlgx' ) ?>:</span>
				<?php echo $the_tags ?>
			</div>
		</div>
	</section>
...
<?php if ( wlgx_get_option( 'post_sharing' ) ) : ?>
	<section class="l-section for_sharing">
		<div class="l-section-h i-cf">
			<?php
			$sharing_providers = (array) wlgx_get_option( 'post_sharing_providers' );
			$wlgx_sharing_atts = array(
				'type' => wlgx_get_option( 'post_sharing_type', 'simple' ),
			);
			foreach ( array( 'email', 'facebook', 'twitter', 'linkedin', 'gplus', 'pinterest', 'vk' ) as $provider ) {
				$wlgx_sharing_atts[$provider] = in_array( $provider, $sharing_providers );
			}
			wlgx_load_template( 'shortcodes/wlgx_sharing', array( 'atts' => $wlgx_sharing_atts ) );
			?>
		</div>
	</section>
<?php endif; ?>

<?php if ( wlgx_get_option( 'post_author_box' ) ): ?>
	<?php wlgx_load_template( 'templates/blog/single-post-author' ) ?>
<?php endif; ?>

<?php if ( wlgx_get_option( 'post_nav' ) AND ! empty( $prevnext ) ): ?>
...
<?php if ( wlgx_get_option( 'post_related', TRUE ) ): ?>
	<?php wlgx_load_template( 'templates/blog/single-post-related' ) ?>
<?php endif; ?>
