...
if ( $has_preview AND ! post_password_required() ) {
	...
	if ( $featured_image == '' ) {
		...
		if ( $post_format == 'image' ) {
			$featured_image = wlgx_get_post_preview( $the_content, TRUE );
			...
			if ( preg_match( '~\[wlgx_gallery.+?\]|\[wlgx_image_slider.+?\]|\[gallery.+?\]~', $the_content, $matches ) ) {
				...
				preg_match( '~\[wlgx_image_slider(.+?)\]~', $gallery, $matches2 );
	...
	if ( in_array(
		...
		$featured_html = wlgx_get_post_preview( $the_content, TRUE );
	}
}
...
$meta_html = apply_filters( 'wlgx_listing_post_meta_html', $meta_html, get_the_ID() );
...
$anchor_atts = '';
// If casestudy with custom link is shown on search
if ( get_post_type() == 'wlgx_casestudy' AND usof_meta( 'wlgx_tile_action' ) == 'link' AND usof_meta( 'wlgx_tile_link' ) != '' ) {
	$link_arr = json_decode( usof_meta( 'wlgx_tile_link' ), TRUE );
	$link = $link_arr['url'];
		...
		<div class="w-blog-post-h">
			<?php if ( $has_preview AND ! empty( $featured_html ) ): ?>
				<div class="w-blog-post-preview">
					...
					<span class="w-blog-post-preview-icon" style="padding-bottom: <?php echo $wlgx_blog_img_ratio; ?>%;"></span>
				</div>
			...
			<div class="w-blog-post-body">
				...
				<?php if ( $show_read_more ): ?>
					<a class="w-blog-post-more w-btn" href="<?php echo $link; ?>"<?php echo $anchor_atts ?>><span class="w-btn-label"><?php _e( 'Read More', 'wlgx' ) ?></span></a>
