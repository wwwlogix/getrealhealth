 ...
 *
 * @action Before the template: 'wlgx_before_template:templates/form/form'
 * @action After the template: 'wlgx_after_template:templates/form/form'
 * @filter Template variables: 'wlgx_template_vars:templates/form/form'
 */
...
global $wlgx_form_index;
// Form indexes start from 1
$wlgx_form_index = isset( $wlgx_form_index ) ? ( $wlgx_form_index + 1 ) : 1;

?>
<div class="w-form<?php echo $classes ?>" id="wlgx_form_<?php echo $wlgx_form_index ?>">
	<form class="w-form-h" autocomplete="off" action="<?php echo esc_attr( $action ) ?>" method="<?php echo $method ?>">
		<?php echo $start_html ?>
		<?php foreach ( $fields as $field ): ?>
			<?php wlgx_load_template( 'templates/form/' . $field['type'], $field ) ?>
		<?php endforeach; ?>
		<div class="w-form-message"></div>
		<?php echo $end_html ?>
	</form>
	<?php if ( isset( $json_data ) AND is_array( $json_data ) AND ! empty( $json_data ) ): ?>
		<div class="w-form-json hidden"<?php echo wlgx_pass_data_to_js( $json_data ) ?>></div>
	<?php endif; ?>
</div>
