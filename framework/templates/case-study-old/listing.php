...
/**
 * Output a single wlgx_casestudy listing. Universal template that is used by all the possible wlgx_casestudy posts listings.
 *
 ...
 * @var $el_class     string Additional classes that will be appended to the main .w-casestudy container
 *
 * @action Before the template: 'wlgx_before_template:templates/casestudy/listing'
 * @action After the template: 'wlgx_after_template:templates/casestudy/listing'
 * @filter Template variables: 'wlgx_template_vars:templates/casestudy/listing'
 */

// .w-casestudy container additional classes and inner CSS-styles
$classes = '';
$inner_css = '';
...
if ( ! empty( $categories ) ) {
	$query_args['wlgx_casestudy_category'] = implode( ',', $categories );
}
...
$wpdb_query .= 'WHERE `term_taxonomy`.`taxonomy` = \'wlgx_casestudy_category\' AND `terms`.`term_id` = `term_taxonomy`.`term_id`';
if ( class_exists( 'SitePress' ) AND defined( 'ICL_LANGUAGE_CODE' ) AND ICL_LANGUAGE_CODE ) {
...
// Categories slugs for all the casestudy items that may be shown by the element
...
if ( empty( $items_categories ) ) {
	if ( ! empty( $categories ) ) {
		...
		// Very unlikely, but still: casestudy posts may be not attached to categories, so fetching them the other way
		// TODO Rewrite the whole algorithm in a more lean way
		wlgx_open_wp_query_context();
		foreach ( get_posts( array( 'post_type' => 'wlgx_casestudy', 'numberposts' => - 1 ) ) as $post ) {
...
if ( count( array_keys( $items_categories ) ) > 0 ) {
	...
	$wpdb_query .= 'WHERE `post_id` IN (' . $items_ids . ') AND `meta_key`=\'wlgx_tile_size\' AND `meta_value` NOT IN (\'\', \'1x1\')';
...
if ( $has_pagination ) {
	...
	$wpdb_query .= 'WHERE `ID` IN (' . $items_ids . ') AND `post_type`=\'wlgx_casestudy\' AND `post_status`=\'publish\' ';
...
if ( $filter == 'category' ) {
	...
	if ( count( $categories_names ) > 1 ) {
		...
		$filter_html .= '<div class="g-filters-item active" data-category="*"><span>' . __( 'All', 'wlgx' ) . '</span></div>';
...
$classes = apply_filters( 'wlgx_casestudy_listing_classes', $classes );

?>


	<div id="<?php echo $el_class ?>" class="g-cols wpb_row type_boxes vc_inner feat-image-casestudy" style="background-image:url('http://grhnew.wpengine.com/wp-content/uploads/2017/01/NYP-1.png?id=5160') !important;"><div class="vc_col-sm-12 wpb_column vc_column_container"><div class="vc_column-inner"><div class="wpb_wrapper"><div class="w-image"><div class="wlgx-frame"><img src="http://grhnew.wpengine.com/wp-content/uploads/2017/01/nyp.svg" class="attachment-full size-full" alt="" /></div></div><h2 style="color: #e2231a;text-align: left;font-family:Abril Fatface;font-weight:400;font-style:normal" class="vc_custom_heading">Our solutions for New York-Presbyterian</h2><div class="wpb_text_column "><div class="wpb_wrapper"><p>I am text block. Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.</p></div> </div> <div class="wpb_text_column "><div class="wpb_wrapper"><p></p></div> </div> <div class="w-btn-wrapper align_left"><a class="w-btn style_outlined color_red icon_none" href=""><span class="w-btn-label">See Case Study</span></a></div></div></div></div></div>
	<div  class="w-casestudy<?php echo $classes ?>"><?php echo $filter_html; ?>
    
    
    
	<div class="w-casestudy-list"><?php
...
if ( $has_pagination ) {
	$json_data = array(
		...
		// casestudy listing template variables that will be passed to this file in the next call
		'template_vars' => array(
			'ratio' => $ratio,
			'metas' => $metas,
			'is_widget' => FALSE,
	...
	?>
<div class="w-casestudy-json hidden"<?php echo wlgx_pass_data_to_js( $json_data ) ?>></div><?php
	if ( $pagination == 'regular' ) {
		...
		<div class="g-loadmore-btn">
			<span><?php _e( 'Load More', 'wlgx' ) ?></span>
		</div>
