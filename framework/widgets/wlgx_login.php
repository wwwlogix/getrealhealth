...
/**
 * wwwlogix Widget: Login
 *
 * Class wlgx_Widget_Login
 */
class wlgx_Widget_Login extends wlgx_Widget {
	...
	function widget( $args, $instance ) {
		...
		$output = '<div class="w-profile-json hidden"' . wlgx_pass_data_to_js( $json_data ) . '></div>';
		...
		$output .= '</a>';
		$output .= '<a class="w-profile-link for_logout" href="">' . __( 'Log Out', 'wlgx' ) . '</a>';
		$output .= '</div>';
		...
		$form_template_vars = array(
			...
			'fields' => array(
				'log' => array(
					'type' => 'textfield',
					'placeholder' => __( 'Username', 'wlgx' ),
					'required' => TRUE,
				),
				'pwd' => array(
					'type' => 'password',
					'placeholder' => __( 'Password', 'wlgx' ),
					'required' => TRUE,
				),
				'submit' => array(
					'type' => 'submit',
					'title' => __( 'Log In', 'wlgx' ),
					'btn_classes' => 'style_raised color_primary',
				),
				'rememberme' => array(
					'type' => 'checkbox',
					'title' => __( 'Remember Me', 'wlgx' ),
					'value' => 'forever',
				),
			),
			'end_html' => '<label for="rememberme"><input id="rememberme" type="checkbox" value="forever" name="rememberme"><span>' . __( 'Remember Me', 'wlgx' ) . '</span></label>',
		);
		...
		if ( $instance['register'] != '' OR $instance['lostpass'] != '' ) {
			...
			if ( $instance['register'] != '' ) {
				...
				$form_template_vars['end_html'] .= __( 'Register', 'wlgx' );
			...
			if ( $instance['lostpass'] != '' ) {
				...
				$form_template_vars['end_html'] .= __( 'Lost Password', 'wlgx' );
				$form_template_vars['end_html'] .= '</a>';
			}
			$form_template_vars['end_html'] .= '</div>';
		}
		wlgx_load_template( 'templates/form/form', $form_template_vars );
