...
(function($){
	var USSidebar = function(){
		...
		this.addWidgetAreaForm = $('#wlgx_add_widget_area');
	...
	USSidebar.prototype = {
		...
		initForm: function(){
			...
			this.nonce = this.widget.find('input[name="wlgx_delete_widget_area_nonce"]').val();
			this.confirmMessage = this.widget.find('#wlgx_confirm_widget_area_deletion').html();
		},

		initCustomWidgets: function(){
			this.widgetsRight.find('.sidebar-wlgx-custom-area').append('<span class="wlgx-custom-area-delete"></span>');
		},

		bindEvents: function(){
			this.widget.on('click', '.wlgx-custom-area-delete', $.proxy(this.deleteWidgetArea, this));
		},
		...
		deleteWidgetArea: function(e){
			...
			$.ajax({
				...
				url: window.ajaxurl,
				data: {
					action: 'wlgx_delete_custom_widget_area',
					name: widgetName,
					_wpnonce: obj.nonce
				},
